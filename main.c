...
/*                                                                                  */
/*  Reseni vytvoril(a) a odevzdava: (Aneta Helesicova, xheles02)                                  */
/*                                                                                  */
/************************************************************************************/
 ...
 * -------------------------------------------------------------------------------- */
#define PB3_ISF_MASK 	0x08
#define PB4_ISF_MASK 	0x10
#define PB5_ISF_MASK 	0x20
#define PB6_ISF_MASK 	0x40
#define PB7_ISF_MASK 	0x80
...
void PORTB_IRQHandler(void) {
	...
	if (/* Doplnte test */ PORTB->PCR[3] & PORT_PCR_ISF_MASK) { /* tlacitko NAHORU (Up) */
		/* zacatek odezvy */
	...
	else if (/* Doplnte test */ PORTB->PCR[4] & PORT_PCR_ISF_MASK) { /* tlacitko PROSTREDNI (Central) */
		/* zacatek odezvy */
	...
	else if (/* Doplnte test */ PORTB->PCR[5] & PORT_PCR_ISF_MASK) { /* tlacitko DOLEVA (Left) */
		/* zacatek odezvy */
		...
		/* konec odezvy */
		/* Vynulujte priznak */
		PORTB->PCR[5]|=PORT_PCR_ISF_MASK;
	}
	else if (/* Doplnte test */ PORTB->PCR[6] & PORT_PCR_ISF_MASK) { /* tlacitko DOLU (Down) */
		/* zacatek odezvy */
		...
		/* konec odezvy */
		/* Vynulujte priznak */
		PORTB->PCR[6]|=PORT_PCR_ISF_MASK;
	}
	else if (/* doplnte test */ PORTB->PCR[7] & PORT_PCR_ISF_MASK) { /* tlacitko DOPRAVA (Right) */
		/* Zacatek odezvy */
...
void init_hardware(void) {
	...
	for(int m=3; m<=7; m++)
	{
		PORTB->PCR[m]&=PORT_PCR_ISF(PORT_PCR_ISF_MASK);
		PORTB->PCR[m]|=PORT_PCR_IRQC(0xA);
		PORTB->PCR[m]|=PORT_PCR_MUX(0x1);
		PORTB->PCR[m]|=PORT_PCR_PE(0x1);
		PORTB->PCR[m]|=PORT_PCR_PS(0x1);
	}
